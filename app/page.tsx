...
function MobileScreen({ platform, activeTab, onTab, theme, onTheme, decision, onDecision, showReason, onReason }: {
          ...
          {activeTab === 'discover' && <div className="discover-screen">
            ...
            <article className={`profile-card card-${decision}`}>
              <div className="profile-image"><div className="story-dots"><i/><i/><i/><i/></div><span className="verified-pill"><BadgeCheck/> Verified</span>
                ...
                <div className="profile-gradient"/><div className="profile-details"><div className="name-line"><h2>Maya, 29</h2><BadgeCheck/></div><p><BriefcaseBusiness/> Product designer</p><p><MapPin/> Brooklyn · 3 miles away</p><div className="profile-tags"><span>Long-term</span><span>Indie films</span><span>Gujarati + English</span></div></div>
              </div>
...
export default function Home() {
  ...
  return <main className={`preview-shell theme-${theme}`}>
    ...
    <section className="compare-stage" id="preview">
      ...
      <div className="device-grid"><MobileScreen platform="ios" activeTab={activeTab} onTab={setActiveTab} theme={theme} onTheme={setTheme} decision={decision} onDecision={setDecision} showReason={showReason} onReason={()=>setShowReason(!showReason)}/><MobileScreen platform="android" activeTab={activeTab} onTab={setActiveTab} theme={theme} onTheme={setTheme} decision={decision} onDecision={setDecision} showReason={showReason} onReason={()=>setShowReason(!showReason)}/></div>

...
import { ArrowLeft, BadgeCheck, Ban, Bot, BriefcaseBusiness, CakeSlice, CalendarHeart, Check, ChevronDown, Compass, EyeOff, Flag, Globe2, Heart, Languages, LocateFixed, MapPin, MessageCircle, MoreHorizontal, Palette, Play, RotateCcw, Ruler, Save, Search, Send, Settings2, ShieldCheck, Sparkles, Star, UserRound, UsersRound, WandSparkles, X, Zap } from 'lucide-react';
...
const interestOptions = ['Travel','Live music','Cooking','Fitness','Films','Books','Art','Hiking'];
function zodiacFor(date:string){const [,monthText,dayText]=date.split('-');const month=Number(monthText),day=Number(dayText);const edge=[20,19,20,20,21,21,22,22,22,22,21,21];const signs=['Capricorn','Aquarius','Pisces','Aries','Taurus','Gemini','Cancer','Leo','Virgo','Libra','Scorpio','Sagittarius','Capricorn'];return signs[month-(day<edge[month-1]?1:0)]||'—'}
const zodiacSymbol:Record<string,string>={Aries:'♈',Taurus:'♉',Gemini:'♊',Cancer:'♋',Leo:'♌',Virgo:'♍',Libra:'♎',Scorpio:'♏',Sagittarius:'♐',Capricorn:'♑',Aquarius:'♒',Pisces:'♓'};
...
function MobileScreen({ platform, activeTab, onTab, theme, onTheme, decision, onDecision, showReason, onReason, profileOpen, onProfileOpen, safetyOpen, onSafetyOpen, blocked, onBlocked, hidden, onHidden, mediaIndex, onMediaIndex, radius, onRadius }: {
  ...
  const [editorOpen,setEditorOpen]=useState(false); const [birthDate,setBirthDate]=useState('1994-10-08');
  const [gender,setGender]=useState('Non-binary'); const [showGender,setShowGender]=useState(true); const [height,setHeight]=useState('170');
  const [interests,setInterests]=useState(['Travel','Films','Cooking']); const [astrology,setAstrology]=useState(true); const [saved,setSaved]=useState(false);
  const zodiac=zodiacFor(birthDate); const completeness=Math.min(100,64+interests.length*5+(height?6:0)+(showGender?5:0)+5);
  const toggleInterest=(item:string)=>setInterests(current=>current.includes(item)?current.filter(value=>value!==item):current.length<5?[...current,item]:current);
  ...
  return <div className={`device-column ${platform}`}>
    ...
    <div className={`phone-frame compare-phone ${platform}`}>
      ...
      <div className="phone-screen">
        ...
        {editorOpen && <section className="profile-builder"><header><button onClick={()=>setEditorOpen(false)} aria-label="Back to profile"><ArrowLeft/></button><span><b>Edit profile</b><small>{saved?'Changes saved':'Visible details are your choice'}</small></span><button className="save-profile" onClick={()=>{setSaved(true);setTimeout(()=>setEditorOpen(false),650)}}><Save/> Save</button></header><div className="builder-scroll"><div className="completion-card"><div><strong>{completeness}%</strong><span>Profile complete</span></div><i><b style={{width:`${completeness}%`}}/></i><p>Add authentic details to improve introductions. Optional fields never block your profile.</p></div><div className="builder-section"><h3>Identity</h3><label><span><UserRound/> Gender identity <small>Used for matching</small></span><NativeSelect size="sm" value={gender} onChange={e=>setGender(e.target.value)}><NativeSelectOption>Woman</NativeSelectOption><NativeSelectOption>Man</NativeSelectOption><NativeSelectOption>Non-binary</NativeSelectOption><NativeSelectOption>Self-described</NativeSelectOption></NativeSelect></label><div className="visibility-row"><span>Show “{gender}” on my profile</span><Switch size="sm" checked={showGender} onCheckedChange={setShowGender}/></div></div><div className="builder-section"><h3>About you</h3><label><span><Ruler/> Height <small>Optional</small></span><div className="unit-input"><Input type="number" min="120" max="230" value={height} onChange={e=>setHeight(e.target.value)}/><b>cm</b></div></label><label><span><CakeSlice/> Date of birth <small>Always private</small></span><Input type="date" value={birthDate} onChange={e=>setBirthDate(e.target.value)}/></label><div className="zodiac-result"><span>{zodiacSymbol[zodiac]}</span><div><b>{zodiac}</b><small>Calculated automatically · Choose whether to display</small></div><Switch size="sm" checked={astrology} onCheckedChange={setAstrology}/></div></div><div className="builder-section"><h3>Interests <small>{interests.length}/5 selected</small></h3><div className="interest-cloud">{interestOptions.map(item=><button key={item} onClick={()=>toggleInterest(item)} aria-pressed={interests.includes(item)}>{interests.includes(item)&&<Check/>}{item}</button>)}</div></div><div className="astrology-card"><div><Sparkles/><span><b>Astrology mode</b><small>Fun context, never a scientific match score.</small></span><Switch size="sm" checked={astrology} onCheckedChange={setAstrology}/></div>{astrology&&<p><strong>{zodiac} {zodiacSymbol[zodiac]} + Libra ♎</strong>{zodiac==='Libra'?'Balanced mirror—similar social energy and communication style.':'Curious connection—different rhythms can create good conversation.'}</p>}</div></div></section>}

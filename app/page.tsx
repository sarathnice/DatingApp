...
import { ArrowLeft, BadgeCheck, Ban, Bot, BriefcaseBusiness, CalendarHeart, Check, ChevronDown, Compass, EyeOff, Flag, Globe2, Heart, Languages, MapPin, MessageCircle, MoreHorizontal, Palette, RotateCcw, Search, Send, Settings2, ShieldCheck, Sparkles, Star, UserRound, UsersRound, WandSparkles, X, Zap } from 'lucide-react';
...
const navigation = [
  { id: 'discover', label: 'Home', icon: Sparkles }, { id: 'explore', label: 'Explore', icon: Compass },
...
function MobileScreen({ platform, activeTab, onTab, theme, onTheme, decision, onDecision, showReason, onReason, profileOpen, onProfileOpen, safetyOpen, onSafetyOpen, blocked, onBlocked, hidden, onHidden }: {
      ...
      <div className="phone-screen">
        ...
        <div className="screen-content">
          {activeTab === 'discover' && <div className="discover-screen">
            ...
            {blocked ? <div className="blocked-state"><span><Ban/></span><h3>Maya is blocked</h3><p>You will not see each other or be able to message.</p><button onClick={() => onBlocked(false)}>Undo block</button></div> : <><article className={`profile-card card-${decision}`}>
              <div className="profile-image"><div className="story-dots"><i/><i/><i/><i/></div><span className="verified-pill"><BadgeCheck/> Verified</span>
                {decision !== 'idle' && <div className="decision-stamp">{decision === 'passed' ? 'Maybe later' : decision === 'intro' ? 'Intro sent' : 'Liked'}</div>}
                <div className="profile-gradient"/><div className="profile-details"><div className="name-line"><h2>Maya, 29</h2><BadgeCheck/></div><p><BriefcaseBusiness/> Product designer</p><p><MapPin/> Brooklyn · 3 miles away</p><div className="profile-tags"><span>Long-term</span><span>Indie films</span><span>Gujarati + English</span></div><button className="view-profile" onClick={() => onProfileOpen(true)}>View full profile <ChevronDown/></button></div>
              </div>
        ...
        {profileOpen && <section className="full-profile"><div className="full-profile-hero"><button className="profile-back" onClick={() => {onProfileOpen(false);onSafetyOpen(false)}} aria-label="Back"><ArrowLeft/></button><button className="profile-more" onClick={() => onSafetyOpen(!safetyOpen)} aria-label="Safety options"><MoreHorizontal/></button>{safetyOpen && <div className="safety-menu"><button onClick={() => {onBlocked(true);onProfileOpen(false);onSafetyOpen(false)}}><Ban/> Block Maya</button><button><Flag/> Report profile</button><button onClick={() => onSafetyOpen(false)}>Cancel</button></div>}<div className="profile-gradient"/><div className="full-name"><h2>Maya, 29</h2><BadgeCheck/><p><MapPin/> Brooklyn · 3 miles away</p></div></div><div className="full-profile-copy"><span className="intent-pill"><Heart/> Looking for a long-term relationship</span><h3>About Maya</h3><p>Product designer, amateur film photographer and a firm believer that the best weekends begin without an itinerary.</p><h3>Two truths and a tiny hill</h3><blockquote>“I make excellent dosa, know every indie cinema in Brooklyn, and think window seats are overrated.”</blockquote><h3>Languages & life</h3><div className="full-tags"><span>English</span><span>Gujarati</span><span>Open to relocate</span><span>Family-minded</span></div></div><div className="full-actions"><button onClick={() => onProfileOpen(false)}><X/> Pass</button><button onClick={() => {onDecision('liked');onProfileOpen(false)}}><Heart/> Like Maya</button></div></section>}
...
export default function Home() {
  ...
  const sharedPreview = {activeTab,onTab:setActiveTab,theme,onTheme:setTheme,decision,onDecision:setDecision,showReason,onReason:()=>setShowReason(!showReason),profileOpen,onProfileOpen:setProfileOpen,safetyOpen,onSafetyOpen:setSafetyOpen,blocked,onBlocked:setBlocked,hidden,onHidden:setHidden};
  return <main className={`preview-shell theme-${theme}`}>
    ...
    <section className="compare-stage" id="preview">
      <div className="compare-heading"><span className="section-kicker"><Globe2/> Worldwide dating, made personal</span><h1>One Mila.<br/><em>Native to both.</em></h1><p>Compare the same product flow across iOS and Android. Choose a page or change the visual theme—the previews update together.</p></div>
